describe ManageIQ::Providers::Autosde::BlockStorageManager do
  ...
  it "has credentials and hostname" do
    ems = FactoryBot.create(:autosde_block_storage_manager, :with_authentication, :name => "kaka")
    expect(described_class.all[0].name).to eq 'kaka'
    expect(ManageIQ::Providers::BlockStorageManager.all[0].authentication_userid).to eq "testuser"
  end

  it "has autosde client" do
    FactoryBot.create(:autosde_block_storage_manager, :with_authentication)
    expect(ManageIQ::Providers::BlockStorageManager.all[0].autosde_client).to be_instance_of ManageIQ::Providers::Autosde::BlockStorageManager::AutosdeClient
  end

  it "can get storage systems " do
    ems = FactoryBot.create(:autosde_block_storage_manager, :with_authentication)
